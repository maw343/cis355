...
class Customer { 
    ...
    function create_record() { // display "create" form
        ...
        $this->generate_form_group("password", $this->passwordError, $this->password_hash);
        $this->generate_html_bottom (1);
    } // end function create_record()
    
    function join_user() { // display "join" form
    ...
    function read_record($id) { // display "read" form
        ...
        $this->generate_form_group("password", $this->passwordError, $this->password_hash, "disabled");
    ...
    function insert_db_record () {        
        if ($this->fieldsAllValid ()) { // validate user input
            ...
            $q = $pdo->prepare($sql);
            $q->execute(array($this->name,$this->email,$this->mobile,$this->password_hash));
            Database::disconnect();
    ...
    function update_db_record ($id) {
        ...
        if ($this->fieldsAllValid()) {
            ...
            $pdo = Database::connect();
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $sql = "UPDATE customers  set name = ?, email = ?, mobile = ?, password_hash = ? WHERE id = ?";
            $q = $pdo->prepare($sql);
            $q->execute(array($this->name,$this->email,$this->mobile,$this->password_hash,$this->id));
            Database::disconnect();
            ...
            <body>
                <div class='container'>
                    <div class='span10 offset1'>
                        ...
                        <form class='form-horizontal' action='customers.php?fun=$funNext' method='post'>                        
                    ";
    ...
    function list_records() {
        ...
        echo "
            ...
            <body>
                ...
                <a href='https://cis355-mawilbur.000webhostapp.com/uploads' target='_blank'>Upload Subdirectory</a><br />
                <div class='container'>
                    <p class='row'> 
                        <h3>$this->title" . "s" . "</h3>
                    </p>
        ...
        echo "
                        ...
                        <form method='post' action='upload01.php' enctype='multipart/form-data'>
                            <p>File</p>
                            <input type='file' name='Filename' accept='image/x-png, image/gif, image/jpeg'> 
                            <br/>
